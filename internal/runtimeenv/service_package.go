...
func (s *PackageService) SetRegistry(ctx context.Context, runtimeName, runtimePath, manager, registry string) error {
	...
	case "python":
		...
		if err != nil {
			...
			return apperror.ErrBadRequest.Wrap(fmt.Errorf("pip config set failed: %s", describeCmdErr(err, output, "pip config set global.index-url")))
		}
		return nil
	default:
		return fmt.Errorf("registry configuration not supported for %s", runtimeName)
...
func (s *PackageService) installNpmPackage(ctx context.Context, req *PackageInstallRequest, runtimePath string) error {
	...
	if manager == "pnpm" {
		if _, lookErr := exec.LookPath(manager); lookErr != nil {
			...
			if _, lookErr2 := exec.LookPath(manager); lookErr2 != nil {
				...
				if installErr != nil {
					return apperror.ErrBadRequest.Wrap(fmt.Errorf("failed to auto-install %s: %w (output: %s)", manager, installErr, installOutput))
				}
				s.miseReshim(ctx)
			}
	...
	if err != nil {
		return apperror.ErrBadRequest.Wrap(fmt.Errorf("%s install failed: %s", manager, describeCmdErr(err, output, manager+" install "+req.Name)))
	}
...
func (s *PackageService) installPipPackage(ctx context.Context, req *PackageInstallRequest, runtimePath string) error {
	...
	if err != nil {
		return apperror.ErrBadRequest.Wrap(fmt.Errorf("pip install failed: %s", describeCmdErr(err, output, "pip install "+req.Name)))
	}
...
func (s *PackageService) uninstallPipPackage(ctx context.Context, req *PackageUninstallRequest, runtimePath string) error {
	...
	if err != nil {
		return apperror.ErrBadRequest.Wrap(fmt.Errorf("pip uninstall failed: %s", describeCmdErr(err, output, "pip uninstall "+req.Name)))
	}
...
func (s *PackageService) updatePipPackage(ctx context.Context, req *PackageUpdateRequest, runtimePath string) error {
	...
	if err != nil {
		return apperror.ErrBadRequest.Wrap(fmt.Errorf("pip update failed: %s", describeCmdErr(err, output, "pip update "+req.Name)))
	}
