...
type sqliteQRLoginRepo struct {
	db *sql.DB
}

func NewQRLoginRepository(db *sql.DB) QRLoginRepository {
	return &sqliteQRLoginRepo{db: db}
}

func (r *sqliteQRLoginRepo) Create(ctx context.Context, s *QRLoginSession) (int64, error) {
...
func (r *sqliteQRLoginRepo) GetByToken(ctx context.Context, qrToken string) (*QRLoginSession, error) {
	s := &QRLoginSession{}
...
func (r *sqliteQRLoginRepo) MarkConfirmed(ctx context.Context, qrToken string, userID int64, webToken string, userJSON string) error {
	_, err := r.db.ExecContext(ctx, `UPDATE qr_login_sessions
		...
		QRStatusConfirmed, userID, webToken, userJSON, time.Now(), qrToken, QRStatusPending)
	if err != nil {
...
func (r *sqliteQRLoginRepo) Consume(ctx context.Context, qrToken string) (*QRLoginSession, bool, error) {
	...
	res, err := r.db.ExecContext(ctx, `UPDATE qr_login_sessions
		...
		QRStatusConsumed, qrToken, QRStatusConfirmed)
	if err != nil {
...
func (r *sqliteQRLoginRepo) Delete(ctx context.Context, qrToken string) error {
	_, err := r.db.ExecContext(ctx, "DELETE FROM qr_login_sessions WHERE qr_token = ?", qrToken)
	return err
}

func (r *sqliteQRLoginRepo) DeleteExpired(ctx context.Context) (int64, error) {
	res, err := r.db.ExecContext(ctx, `DELETE FROM qr_login_sessions
		WHERE expires_at < ? OR status IN (?, ?)`, time.Now(), QRStatusCancelled, QRStatusConsumed)
	if err != nil {
		return 0, err
	}
