...
func Setup(cfg *config.Config, configPath string, sig *infra.Signal) (http.Handler, func()) {
	...
	systemdhttp.RegisterRoutes(g.Protected, g.WS, serviceManager, cmdExec, cfg.Auth.JWTSecret, auditSvc, cfg.Server.AllowedOrigins, cfg.Server.DevMode)
	...
	audithttp.RegisterRoutes(g.Protected, db, auditSvc, auditRepo)
