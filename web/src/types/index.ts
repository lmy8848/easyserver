...
export interface AppSettings {
  server: { port: number; host: string; tls: { enabled: boolean; cert_info: TLSCertInfo | null }; domain: string; force_domain: boolean; max_upload_size: number; assets_rate_limit: number; assets_rate_interval: string; turnstile: { site_key: string; secret_key: string; enable_login: boolean; enable_qr_login: boolean; enable_public_share: boolean } };
  auth: { session_timeout: number; idle_timeout: number; max_login_attempts: number; lockout_duration: number; rate_limit: number; rate_interval: number; login_rate_limit: number; login_rate_interval: number; allow_multi_session: boolean };
